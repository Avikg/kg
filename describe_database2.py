...
Annotations include tissue of origin, SIFT functional impact prediction (D=Deleterious/T=Tolerated),
...
cmc = pd.read_csv(
    ...
    usecols=["GENE_SYMBOL","MUTATION_DESCRIPTION","TIER",
             "SIGNIFICANCE_TIER","SAMPLES_MUTATED","SAMPLES_TESTED","SIFT_PRED"])
print(cmc["MUTATION_DESCRIPTION"].value_counts())
# Tier-1 pathogenic mutations per gene
deleterious = cmc[cmc["SIFT_PRED"]=="D"]
print(deleterious["GENE_SYMBOL"].value_counts().head(10))
...
cgc_schema = [
    ...
    ("MUTATION_SOMATIC_STATUS","string","Confirms the mutation is somatic (not germline). Key quality filter for cancer ML models.","Confirmed somatic variant","Somatic filter — remove germline variants that are not cancer mutations"),
...
cgc = pd.read_csv(
    ...
    usecols=["GENE_SYMBOL","MUTATION_DESCRIPTION","MUTATION_CDS","MUTATION_AA",
             "CHROMOSOME","GENOME_START","MUTATION_SOMATIC_STATUS"])
print(cgc["MUTATION_DESCRIPTION"].value_counts())
...
<div class="sec"><div class="sec-title">Database Cards</div><div class="cards c2">
  ...
  <div class="card" style="border-color:{ccmc}55">
    ...
    <div style="font-size:12px;color:#aaa;margin-bottom:10px">12 columns · SIFT pathogenicity · TIER · free academic</div>
...
<div class="sec"><div class="sec-title">Why Each Database Matters</div>
<div class="cards c2">
  <div class="card" style="border-left:3px solid #4C9BE8">
    <div style="font-size:13px;font-weight:600;color:#4C9BE8;margin-bottom:8px">CancerMine — Literature Evidence</div>
    <div style="font-size:12px;color:#ccc;line-height:1.7">Provides the <b>functional role labels</b> for each gene-cancer pair. Without CancerMine, we would have mutation data but no understanding of whether a mutation is a driver, oncogene, or tumour suppressor. It is the <em>semantic layer</em> of the KG.</div>
  </div>
  <div class="card" style="border-left:3px solid #E8724C">
    <div style="font-size:13px;font-weight:600;color:#E8724C;margin-bottom:8px">COSMIC — Biological Evidence</div>
    <div style="font-size:12px;color:#ccc;line-height:1.7">Provides <b>experimental mutation-level data</b> from 1.4M real patient tumours. Without COSMIC, we would have literature claims but no direct genomic evidence. It is the <em>experimental validation layer</em> of the KG.</div>
  </div>
  <div class="card" style="border-left:3px solid #52B788">
    <div style="font-size:13px;font-weight:600;color:#52B788;margin-bottom:8px">CGC — Validated Driver Set</div>
    <div style="font-size:12px;color:#ccc;line-height:1.7">Provides the <b>gold-standard list</b> of 723 proven cancer driver genes with Tier 1/2 confidence. It acts as the <em>quality filter</em> — genes in the CGC are the highest-priority targets for the ML prediction model.</div>
  </div>
  <div class="card" style="border-left:3px solid #8E6BBF">
    <div style="font-size:13px;font-weight:600;color:#8E6BBF;margin-bottom:8px">HeNeCOn — Clinical Semantics</div>
    <div style="font-size:12px;color:#ccc;line-height:1.7">Provides <b>formal clinical definitions</b> for HNC concepts — staging, anatomy, treatment. Without HeNeCOn, the KG cannot bridge gene evidence to clinical decision-making. It is the <em>clinical knowledge layer</em>.</div>
  </div>
</div></div>
<div class="sec"><div class="sec-title">How the Databases Connect — Integration Logic</div>
<div class="card">
<div style="font-size:13px;color:#ccc;line-height:1.8;margin-bottom:14px">
The three databases are integrated via <b>shared entity labels</b>: gene symbols (e.g. TP53) appear in both CancerMine and COSMIC.
Cancer type names (e.g. "head and neck cancer") appear in CancerMine and map to HeNeCOn ontology classes.
This creates a unified multi-relational graph where a <em>single query</em> can answer:
<em>"Which genes are literature-confirmed drivers of HNC, also have somatic mutations in head tissue, and map to a formal staging concept?"</em>
— impossible with any single database alone.
</div>
<table class="data" style="min-width:auto">
<thead><tr><th>Question</th><th>Requires</th><th>How linked</th></tr></thead>
<tbody>
<tr><td>Which genes drive cancer X?</td><td>CancerMine</td><td>gene_normalized × cancer_normalized</td></tr>
<tr><td>How often is gene X mutated in tissue Y?</td><td>COSMIC</td><td>GENE_SYMBOL × PRIMARY_SITE</td></tr>
<tr><td>Is gene X a confirmed cancer driver?</td><td>COSMIC CGC</td><td>Tier 1 = gold standard</td></tr>
<tr><td>What does cancer X mean clinically?</td><td>HeNeCOn</td><td>rdfs:label match to cancer node</td></tr>
<tr><td>All of the above simultaneously</td><td>Combined KG</td><td>Shared gene/cancer labels across all 3</td></tr>
</tbody></table>
</div></div>
